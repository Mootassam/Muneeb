  ...
  return (
    <div className="withdraw-list-container">
          ...
          <table className="withdraw-list-table">
            <thead className="table-header">
              <tr>
                ...
                <th className="actions-header withdraw-table-actions-header">
                  Actions
                </th>
              </tr>
            </thead>
                ...
                rows.map((row) => (
                  <tr key={row.id} className="table-row">
                    <td className="table-cell">
                      <div className="withdraw-user-cell">
                        {row.user ? (
                          <div className="withdraw-user-info">
                            <div className="withdraw-user-name">
                              {row.user.fullName || row.user.email}
                            </div>
                            {row.user.email && (
                              <div className="withdraw-user-email">{row.user.email}</div>
                            )}
                          </div>
                        ) : (
                    ...
                    <td className="table-cell">
                      <div className="withdraw-address-cell">
                        <span className="withdraw-address-text" title={row.address}>
                          {row.address}
                        </span>
                        {row.address && (
                          <button
                            type="button"
                            className="withdraw-address-copy"
                      ...
                      <div className="withdraw-protocol">
                        {row.currency} · {row.protocol}
                      </div>
                    </td>
                    ...
                    <td className="table-cell">
                      <div className="withdraw-date">
                        <div className="withdraw-date-day">
                          {row.createdAt ? new Date(row.createdAt).toLocaleDateString() : '-'}
                        </div>
                        <div className="withdraw-date-time">
                          {row.createdAt ? new Date(row.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                        </div>
                      </div>
                    </td>
                    <td className="withdraw-table-actions">
                      <div className="withdraw-table-actions-content">
                        <Link
                          className="withdraw-table-action-btn info"
                          to={`/withdraw/${row.id}`}
                        >
                          <i className="fas fa-eye withdraw-table-action-icon" />
                          View
                        </Link>
                        {hasPermissionToEdit && (
                          <Link
                            className="withdraw-table-action-btn primary"
                            to={`/withdraw/${row.id}/edit`}
                          >
                            <i className="fas fa-edit withdraw-table-action-icon" />
                            Edit
                          </Link>
                        )}
                        {hasPermissionToDestroy && (
                          <button
                            className="withdraw-table-action-btn danger"
                            onClick={() => setRecordIdToDestroy(row.id)}
                          >
                            <i className="fas fa-trash withdraw-table-action-icon" />
                            Delete
                          </button>
                        )}
                      </div>
      ...
      <style>{`
        ...
        .withdraw-table-actions-header {
          background: #f8fafc;
        }

        .withdraw-table-actions {
          position: sticky;
          right: 0;
          background: white;
          z-index: 10;
        ...
        .withdraw-table-actions-content {
          display: flex;
          align-items: center;
          gap: 6px;
          flex-wrap: nowrap;
        ...
        .withdraw-table-action-btn:hover {
          transform: translateY(-1px);
          box-shadow: 0 4px 8px rgba(0,0,0,0.15);
        }

        .withdraw-table-action-btn.primary {
          background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
          color: white;
        }

        .withdraw-table-action-btn.danger {
          background: linear-gradient(135deg, #f56565 0%, #e53e3e 100%);
          color: white;
        }

        .withdraw-table-action-btn.info {
          background: linear-gradient(135deg, #4299e1 0%, #3182ce 100%);
          color: white;
        }

        .withdraw-table-action-icon {
          margin-right: 4px;
          font-size: 10px;
          width: 10px;
        }
