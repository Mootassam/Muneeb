...
const schema = yup.object().shape({
  ...
  protocol: yupFormSchemas.enumerator(
...
function DepositForm(props) {
  const [initialValues] = useState(() => {
    ...
    return {
      ...
      currency: record.currency || depositEnumerators.currency[0],
      paymentMethod: record.paymentMethod || depositEnumerators.paymentMethod[0],
      protocol: record.protocol || depositEnumerators.protocol[0],
    };
  });
          ...
          <div className="row">
            <div className="col-lg-7 col-md-8 col-12">
              <SelectFormItem
                ...
                options={depositEnumerators.status.map(
                  (value) => ({
                    value,
                    label: i18n(
                      `entities.deposit.enumerators.status.${value}`,
                    ),
                  }),
                )}
                required={true}
            ...
            <div className="col-lg-7 col-md-8 col-12">
              <SelectFormItem
                name="currency"
                label={i18n(
                  'entities.deposit.fields.currency',
                )}
                options={depositEnumerators.currency.map(
                  (value) => ({
                    value,
                    label: value,
                  }),
            ...
            <div className="col-lg-7 col-md-8 col-12">
              <SelectFormItem
                name="paymentMethod"
                label={i18n(
                  'entities.deposit.fields.paymentMethod',
                )}
                options={depositEnumerators.paymentMethod.map(
                  (value) => ({
                    value,
                    label: value,
                  }),
            ...
            <div className="col-lg-7 col-md-8 col-12">
              <SelectFormItem
                name="protocol"
                label={i18n(
                  'entities.deposit.fields.protocol',
                )}
                options={depositEnumerators.protocol.map(
                  (value) => ({
                    value,
                    label: value,
                  }),
